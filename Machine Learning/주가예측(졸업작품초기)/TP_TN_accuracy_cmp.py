...
##############################################
a, raw_data = read_file('amore.csv')
...
for i in range(0, len(y) - seq_length):
    _x = x[i:i + seq_length]
    _y = y[i + seq_length]# 5days close price
    #print(_x, "->", _y)
    dataX.append(_x)
    dataY.append(_y)
...
with tf.Session() as sess:
    ...
    for i in range(0, len(test_predict)):
        if i == 0:
            continue
        elif test_predict[i] >= test_predict[i - 1] and testY[i] >= testY[i - 1]:#TP
            TP = TP + 1
        elif test_predict[i] < test_predict[i - 1] and testY[i] < testY[i - 1]:
            TN = TN + 1
        elif test_predict[i] >= test_predict[i - 1] and testY[i] <= testY[i - 1]:
            FP = FP + 1
        elif test_predict[i] < test_predict[i - 1] and testY[i] > testY[i - 1]:
...
#   print("RMSE: {}".format(rmse_val))
    print("Positive precision : ", (TP/(TP+FP)) * 100,"%")
    print("Negative precision : ", (TN/(TN + FN)) * 100,"%")
    print("accuracy : ", ((TP + TN)/(TP+FP+TN+FN)) * 100,"%") 
    
    # Plot predictions
    plt.plot(testY[1195:1200])
    plt.plot(test_predict[1195:1200])
